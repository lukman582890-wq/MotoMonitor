...
const replacement = `async function connectBmsDevice(d){try{st.bms.id=d.deviceId;st.bms.name=d.name||d.deviceId;await BleClient.connect(d.deviceId,()=>{st.bms.connected=false;render();log('JK BMS disconnected')});await BleClient.startNotifications(d.deviceId,BMS_SERVICE,BMS_CHAR,v=>jkData(Array.from(new Uint8Array(v.buffer,v.byteOffset,v.byteLength))));st.bms.connected=true;jb=new Uint8Array(0);render();log(\`JK BMS connected: \${st.bms.name}\`);for(const cmd of [150,151]){const q=new Uint8Array(20);q.set([170,85,144,235,cmd]);let sum=0;for(let i=0;i<19;i++)sum=(sum+q[i])&255;q[19]=sum;traceRaw('JK TX',q);log(\`JK TX command 0x\${cmd.toString(16).toUpperCase()}\`);await BleClient.writeWithoutResponse(d.deviceId,BMS_SERVICE,BMS_CHAR,new DataView(q.buffer)).catch(e=>log(\`JK TX 0x\${cmd.toString(16).toUpperCase()} error: \${e?.message||e}\`));await new Promise(r=>setTimeout(r,500))}}catch(e){st.bms.connected=false;render();log(\`JK BMS error: \${e?.message||e}\`)}}
...
console.log('JK BMS picker updated: request realtime 0x96 first, then 0x97 info.');
