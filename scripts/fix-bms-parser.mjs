...
const replacement = `function parse55(b){
  if(b.length<300||b[0]!=85||b[1]!=170||b[2]!=235||b[3]!=144)return;
  if(b[4]!==2)return;
  ...
  for(let i=0;i<32&&6+2*i+1<299;i++){
    ...
    if(v>=1&&v<=5)cells.push(v); else if(i>=10)break;
  }
  if(!cells.length)return;
  st.bms.cells=cells;
  const sum=cells.reduce((a,v)=>a+v,0);
  st.bms.delta=Math.max(...cells)-Math.min(...cells);
  const is32=cells.length>24;
  const o=is32?32:0;
  const total=u32le(b,124+o);
  const current=i32le(b,132+o);
  const temp=i16le(b,136+o);
  const temp2=i16le(b,138+o);
  const soc=b[147+o];
  if(total>10000&&total<200000)st.bms.voltage=total/1000;
  else st.bms.voltage=sum;
  if(Number.isFinite(current)&&Math.abs(current)<500000)st.bms.current=current/1000;
  if(Number.isFinite(temp)&&temp>-1000&&temp<1500)st.bms.temp=temp/10;
  else if(Number.isFinite(temp2)&&temp2>-1000&&temp2<1500)st.bms.temp=temp2/10;
...
console.log('JK BMS runtime parser fixed for 300-byte 55AAEB90 frames.');
